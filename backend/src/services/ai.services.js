...
import { getAIContext } from "./registry.service.js";
...
export const buildGenerationProfileFromMeta = (meta = {}) => {
  const board = meta?.board || DEFAULT_BOARD_KEY;
  const boardProfile = getBoardProfileFromMeta({ board });
  ...
  return {
    board,
    boardPartType: boardProfile.boardPartType,
    powerSource,
    language,
...
export const detectBoardFromRequirements = (requirements = []) => {
  ...
  if (text.includes("mega")) return "arduino-mega";
  if (text.includes("nano")) return "arduino-nano";
  if (text.includes("esp32")) return "esp32-devkit-v1";
  if (text.includes("pico")) return "raspberry-pi-pico";
...
`;
  ...
  return {
    ...catalogSafe,
    detectedMeta: {
      board: detectBoardFromRequirements(guarded.requirements),
